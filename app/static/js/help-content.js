      ...
      },
      {
        ...
        body: `
          ...
          <ul>
            ...
            <li><strong>Propositor grounded</strong> — a IA que propõe recebe o contexto do agente + resumo do gold (categorias, contagens, decisões esperadas) + falhas capturadas (output + motivo), mas NUNCA as respostas-gabarito (<code>expected_output</code>/<code>expected_pattern</code>), os literais de red flag, nem QUALQUER caso do holdout (anti-vazamento). Papel de LLM próprio ("optimizer") — recomendado DIFERENTE do juiz (anti-Goodhart: o mesmo modelo propondo e julgando seleciona prompts que agradam a si mesmo; a UI avisa quando isso acontece).</li>
            ...
            <li><strong>Teto de custo (US$)</strong> — verificado entre passos; um passo em voo pode estourar o teto em até ~1 passo. Deixar 0 na tela aplica o padrão da plataforma (<code>optimizer_default_budget_usd</code> — que por default é 0 = sem teto; ajuste em Configurações → Parâmetros se quiser um teto global).</li>
