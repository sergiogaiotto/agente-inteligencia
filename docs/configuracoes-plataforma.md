...
| MCP per-tool | `mcp_per_tool_enabled` / `MCP_PER_TOOL_ENABLED` | **false** | Cada tool MCP vira função própria com schema real (vs legado `{operation,query}`). Requer `discovered_tools`. Vale em runtime. **39.0.0**: compõe com o override POR CONECTOR (`Modo per-tool`: Herdar global / Ligado / Desligado, no form de /mcp) — piloto num conector só, ou opt-out pontual, sem mexer na frota. |
